           ...
           <HeadingTitle title='Coupons & Promos' />
            <View>
              <ScrollView horizontal contentContainerStyle={{ paddingHorizontal: mvs(18) }}>
                {[0, 1, 2, 3, 4].map((ele, index) => (<TouchableOpacity onPress={()=>props?.navigation?.navigate('CouponDetails')} style={{ width: mvs(134), marginRight: mvs(7), borderTopRightRadius: mvs(8), borderRadius: mvs(8), overflow: 'hidden' }}>
