...
import { CarWash, Map, Ratings, RightArrow, Total } from '../../assets/common-icons';
                ...
                <ScrollView contentContainerStyle={{ flexGrow: 1, }}>
                    ...
                    <Row style={{ marginTop: mvs(17) }}>
                        <ScrollView horizontal contentContainerStyle={{ paddingHorizontal: mvs(18) }}>
                            {services.map((item, index) => <ServiceCard middleText={index === 0 ? '4.1' : null} value={index === 0 ? null : item.value} title={item.title} icon={item.icon} div={services.length - 1 !== index} />)}
                        </ScrollView>
                    </Row>
...
export default connect(mapStateToProps, mapDispatchToProps)(ServiceOfferingDetails);
