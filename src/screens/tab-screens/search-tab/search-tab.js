...
const Search = (props) => {
  ...
  const renderHomeCard = ({ item, index }) => {
    return (
      <TouchableOpacity onPress={() => props?.navigation?.navigate('CategoryDetails', { item })} style={styles.card}>
    ...
    <View style={{ ...styles.conntainer, backgroundColor: colors.background }}>
      ...
      <View style={styles.body}>
        <View style={{ marginVertical: mvs(20) }}>
          <ScrollView horizontal contentContainerStyle={{ paddingHorizontal: mvs(22) }}>
            {home_categories?.data?.map((ele,key) =>
              <Buttons.ButtonPrimary key={key}  onClick={() => setSelectedCat(ele?.id)} textStyle={{ color: ele?.id === selectedCat ? colors?.white : colors.primary }} style={{ backgroundColor: ele?.id === selectedCat ? colors.primary : colors.card, width: null, height: mvs(38), paddingHorizontal: mvs(10), marginRight: mvs(10) }} title={ele?.name} />
            )}
          </ScrollView>
        </View>
        <FlatList
