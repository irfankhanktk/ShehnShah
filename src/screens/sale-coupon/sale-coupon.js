...
const SaleCoupon = props => {
  const {get_coupon_sale_details,route,update_coupon_payment,complete_coupon_purchase}=props;
  ...
  const purchaseCoupon=async()=>{
    ...
    const completeResponse=await complete_coupon_purchase(id,saleData?.couponId)
    console.log("payment Response",completeResponse?.data)
    setpayload({...payload, updateStart: false});
  }
  return (
    <View style={{ ...styles.container, backgroundColor: colors.white }}>
      <CustomHeader colors={colors} title={saleData?.customer?.name} titleStyle={{color:colors.black}} allowBackBtn spacebetween/>
       <ScrollView contentContainerStyle={{ flexGrow: 1, paddingHorizontal: mvs(16) }}>
         <View style={styles.body}>
            ...
            <Row style={{...styles.TIMETOPVIEW,paddingHorizontal:0,borderBottomWidth:0,paddingBottom:5}}> 
                 <View>
                      <Bold label={"Payment Method"} size={15}/>
                      <Regular label={payMethod=="Cash"?"Cash on Delivery" : "Online payment"} size={16} numberOfLines={1}/>
                 </View>
                 <View style={{flexDirection:'column',justifyContent:'flex-end',height:mvs(40)}}>
                    <Bold label={saleData?.invoice?.total+" AED"} size={16} color={colors.primary} 
               ...
               <View style={styles.paymentView}>
                   <BookingPaymentView Vat={saleData?.invoice?.vat} subTotal={saleData?.invoice?.subTotal} total={saleData?.invoice?.total}/>
...
const mapDispatchToProps = {
    get_coupon_sale_details: (cId,sId) => DIVIY_API.get_coupon_sale_details(cId,sId),
